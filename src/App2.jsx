...
// Message Component
// -----------------------
// Modify the component so that if the message is from the assistant and
// its audio is currently playing (i.e. speakingMessageId matches the index),
// then do not show the full text until audio finishes.
    ...
    <div className={`flex items-end ${msg.sender === "user" ? "flex-row-reverse" : ""}`}>
      ...
      <div className="mx-2 max-w-md">
        <div className={`rounded-lg p-3 shadow-md relative transition-all duration-500 ${msg.sender === "user" ? "bg-[#7886C7] text-white" : "bg-[#2D336B] text-white"}`}>
          {
            // If this is an assistant message and it’s still being spoken,
            // hide the text (or you could show a placeholder).
            msg.sender === "assistant" && speakingMessageId === index 
              ? null 
              : msg.text
          }
...
const InterviewChat = ({ token, username, onLogout }) => {
  ...
  const handleSpeechResult = async (finalText) => {
    ...
    try {
      ...
      // Add the assistant's message. The text will be hidden until audio playback finishes.
      ...
      // Compute the index of the assistant message that was just added.
      const msgIndex = messages.length; // assuming messages.length was the index of the new assistant message
      audio.onplay = () => setSpeakingMessageId(msgIndex);
      audio.onended = () => {
        setSpeakingMessageId(null); // Now that the audio is done, the text will be revealed.
        audioRef.current = null;
      };
      try {
        await audio.play();
  ...
  // -----------------------
  // Modified startListening to use continuous mode
  // -----------------------
  const startListening = () => {
    ...
    recognition.continuous = true; // Change to continuous listening
    ...
    // When recognition ends, restart it if the user is still listening.
    recognition.onend = () => {
      if (listening) {
        recognition.start();
      }
  ...
  const handleMicClick = () => {
    ...
    if (!listening) {
      // (1) If not listening, stop any playing audio and start recognition.
      stopAudioPlayback();
      if (recognitionRef.current) {
        recognitionRef.current.stop();
      }
      ...
      // (2) If listening, stop recognition and process the final transcript.
      setListening(false);
      if (recognitionRef.current) {
        recognitionRef.current.stop();
      }
